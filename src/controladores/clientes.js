...
const registrarCliente = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ mensagem: "Cliente cadastrado com sucesso.", cliente })
  ...
  try {
    ...
    if (email) {
      const clienteExistente = await knex('clientes')
        .where({ email }).whereNot({ id: id }).first()

      if (clienteExistente) {
        return res.status(400).json("O email já está cadastrado para outro cliente")
      }
      body.email = email;
    }
    ...
    if (cpf) {
      ...
      const verificaCPF = await knex('clientes').where({ cpf }).count('id as count')
    ...
    const clienteAtualizado = await knex('clientes').where({ id }).update(body).returning('*')
